...
function TaskDetailsDrawer({ task, sprints, onClose }: TaskDetailsDrawerProps) {
  ...
  return (
    <div
      className={`fixed inset-y-0 right-0 z-50 w-full max-w-xl border-l border-slate-200 bg-white p-6 shadow-2xl transition-transform duration-300 dark:border-slate-700 dark:bg-slate-800 ${
        ...
        <div className="flex h-full flex-col">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="truncate font-mono text-xs font-semibold uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400">
                {task.ticketKey}
              </p>
              <h2 className="mt-2 text-2xl font-bold leading-8 text-slate-950 dark:text-white">{task.title}</h2>
            </div>
            <button
              type="button"
              onClick={onClose}
